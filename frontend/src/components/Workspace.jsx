...
export default function Workspace({ onGenerated }) {
  ...
  useEffect(() => {
    const loadPrefill = () => {
      try {
        const t = window.localStorage.getItem("aiMemoryPrefill");
        if (t && t.trim()) {
          setTranscript(t);
    ...
    el.addEventListener("ai-memory-prefill", loadPrefill);
    return () => el.removeEventListener("ai-memory-prefill", loadPrefill);
  }, []);
  ...
  return (
    <div data-testid="workspace" className="animate-fade-up">
      <div className="rounded-md border border-border bg-card">
        <div className="border-b border-border p-6">
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            Paste the conversation where you hit a context wall. The engine reconstructs the full
            project state so any other AI can continue seamlessly.
          </p>
        </div>
          ...
          <div className="grid gap-5 md:grid-cols-3">
            ...
            <div>
              ...
              <Select value={sourceAi} onValueChange={setSourceAi}>
                <SelectTrigger data-testid="source-ai-select" className="mt-2 bg-background">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {AI_OPTIONS.map((o) => (
                    <SelectItem key={o} value={o} data-testid={`source-ai-${o}`}>{o}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div>
              ...
              <Select value={targetAi} onValueChange={setTargetAi}>
                <SelectTrigger data-testid="target-ai-select" className="mt-2 bg-background">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {AI_OPTIONS.map((o) => (
                    <SelectItem key={o} value={o} data-testid={`target-ai-${o}`}>{o}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
            ...
            >
              {loading ? (
                ...
                <>
                  <Sparkles className="h-4 w-4" /> Generate Memory Package
